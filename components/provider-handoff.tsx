...
export default function ProviderHandoff() {
  ...
  useEffect(() => {
    ...
    const launch = async () => {
      try {
        ...
        if (!accessToken) {
          throw new Error('Your Skooby session could not be verified.');
        }
        ...
        if (!response.ok || !payload.redirectUrl) {
          throw new Error(payload.error || 'Provider connection is unavailable.');
        }
  ...
  const displayError =
    ready && !authenticated
      ? 'Sign in with Skooby before opening the provider.'
      : error;
    ...
    <main className="flex min-h-screen items-center justify-center bg-[#050b08] px-6 text-white">
      <div className="text-center">
        ...
        <p className="mt-2 text-sm text-white/50">
          The provider destination and return state are being verified before transfer.
        </p>
        {user?.wallet?.address ? (
          <p className="mx-auto mt-3 max-w-xs truncate text-[11px] text-white/30">
