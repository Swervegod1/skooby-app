    ...
    <div className="overflow-hidden rounded-[2rem] border border-white/10 bg-[#0a1511]/95 shadow-2xl shadow-black/30">
      ...
      <div className="grid lg:grid-cols-[210px_1fr_320px]">
        <aside className="border-b border-white/10 p-4 lg:border-b-0 lg:border-r">
          ...
          <div className="space-y-1">{games.map((game) => <button key={game.id} onClick={() => { setSelectedId(game.id); setResult(null); setError(null); }} className={`flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-left text-sm font-black transition ${selected.id === game.id ? 'bg-lime-300 text-black' : 'text-white/60 hover:bg-white/[0.06]'}`}><span className="text-xl">{game.icon}</span>{game.name}</button>)}</div>
        </aside>
        ...
        <section className="p-5 sm:p-8">
          ...
          <div className="mt-7 rounded-[2rem] border border-white/10 bg-black/30 p-5 sm:p-7">
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="block"><span className="text-[10px] font-black uppercase tracking-[0.18em] text-white/35">Play-credit amount</span><input type="number" min="1" step="1" value={bet} onChange={(event) => setBet(Math.max(1, Number(event.target.value) || 1))} className="mt-2 w-full rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-3 font-bold outline-none" /></label>
        ...
        <aside className="space-y-4 border-t border-white/10 p-5 lg:border-l lg:border-t-0">
          ...
          <section className="rounded-3xl border border-white/10 bg-white/[0.03] p-5"><p className="text-[10px] font-black uppercase tracking-[0.2em] text-lime-200">Live rankings</p><div className="mt-3 space-y-2">{leaders.slice(0, 8).map((leader) => <div key={`${leader.rank}-${leader.player}`} className="flex justify-between text-xs text-white/55"><span>#{leader.rank} {leader.player}</span><span className="font-black text-white/75">{leader.credits.toFixed(2)}</span></div>)}{!leaders.length ? <p className="text-xs text-white/35">Leaderboard will appear when the secure credit store is configured.</p> : null}</div></section>
          <section className="rounded-3xl border border-white/10 bg-white/[0.03] p-5 text-xs leading-5 text-white/45">Browser state is display-only. Balance changes are accepted only from authenticated game APIs or signed provider webhooks and are persisted server-side.</section>
        </aside>
      </div>
    </div>
  );
...
export function CasinoHub() {
  if (!process.env.NEXT_PUBLIC_PRIVY_APP_ID) {
    return (
      <div className="grid gap-5 rounded-[2rem] border border-white/10 bg-[#0a1511]/95 p-6 lg:grid-cols-[1fr_320px]">
        <div><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-200">Authentication required</p><h2 className="mt-3 text-3xl font-black">Server-authoritative play is locked.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-white/45">Configure Privy and the server verification key before enabling game mutations. The application fails closed instead of trusting browser state.</p></div>
        <ProvablyFairVerifier />
      </div>
    );
  }
  return <ConfiguredCasinoHub />;
}
